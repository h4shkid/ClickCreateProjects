/**
 * Test ClickCreate Historical Snapshots
...
const CLICKCREATE_ADDRESS = '0x300e7a5fb0ab08af367d5fb3915930791bb08c2b'

async function testClickCreateSnapshots() {
  console.log('🎨 Testing ClickCreate Historical Snapshots\n')

  const db = createDatabaseAdapter()
  const converter = createDateToBlockConverter()

  // Test 1: First 10 tokens at early date
  console.log('=' .repeat(60))
  console.log('📅 TEST 1: Tokens 1-10 on March 1, 2023')
  console.log('=' .repeat(60))

  const date1 = new Date('2023-03-01T00:00:00Z')
  const block1 = await converter.dateToBlock(date1)
  const actualDate1 = await converter.blockToDate(block1)

  console.log(`\nRequested Date: 2023-03-01`)
  ...
  const holders1 = await getHistoricalSnapshot(db, CLICKCREATE_ADDRESS, block1, '1-10', false)
  ...
  // Test 2: All tokens (1-97) exact match at recent date
  console.log('\n\n' + '='.repeat(60))
  console.log('📅 TEST 2: All Tokens (1-97) Exact Match on January 1, 2024')
  console.log('=' .repeat(60))
  ...
  console.log(`   Token IDs: 1-97`)
  console.log(`   Exact Match: YES`)

  const holders2 = await getHistoricalSnapshot(db, CLICKCREATE_ADDRESS, block2, '1-97', true)
  ...
  if (holders2.length > 0) {
    ...
    console.log(`   (No holders with ALL 97 tokens)`)
  }

  // Test 3: Date range comparison (Jan 2024 - Jun 2024)
  console.log('\n\n' + '='.repeat(60))
  console.log('📅 TEST 3: Tokens 1-50 - Date Range (Jan 1 to Jun 1, 2024)')
  console.log('=' .repeat(60))
  ...
  console.log(`\nRequested Range: 2024-01-01 to 2024-06-01`)
  ...
  console.log(`   Start Date: 2024-01-01`)
  console.log(`   End Date: 2024-06-01`)
  console.log(`   Token IDs: 1-50`)
  console.log(`   Exact Match: NO`)

  const startHolders = await getHistoricalSnapshot(db, CLICKCREATE_ADDRESS, startBlock, '1-50', false)
  const endHolders = await getHistoricalSnapshot(db, CLICKCREATE_ADDRESS, endBlock, '1-50', false)

  console.log(`\n✅ Expected Result:`)
  console.log(`   Start (Jan 1): ${startHolders.length} holders, ${startHolders.totalSupply} tokens`)
  console.log(`   End (Jun 1): ${endHolders.length} holders, ${endHolders.totalSupply} tokens`)
  console.log(`   Change: ${endHolders.length - startHolders.length} holders, ${endHolders.totalSupply - startHolders.totalSupply} tokens`)

  // Test 4: Specific tokens with exact match
  console.log('\n\n' + '='.repeat(60))
  console.log('📅 TEST 4: Tokens 10-20 Exact Match on October 1, 2024')
  console.log('=' .repeat(60))

  // Use approximate block for Oct 1, 2024 to avoid RPC rate limits
  const block4 = 20900000 // Approximately Oct 1, 2024
  const actualDate4 = new Date('2024-10-01T00:00:00Z')

  console.log(`\nRequested Date: 2024-10-01`)
  ...
  console.log(`   Token IDs: 10-20`)
  console.log(`   Exact Match: YES`)

  const holders4 = await getHistoricalSnapshot(db, CLICKCREATE_ADDRESS, block4, '10-20', true)
  console.log(`\n✅ Expected Result: ${holders4.length} holders with ${holders4.totalSupply} total tokens`)
  if (holders4.length > 0) {
    console.log(`   Top 3 holders:`)
    holders4.holders.slice(0, 3).forEach((h: any, i: number) => {
      console.log(`   ${i + 1}. ${h.holderAddress.slice(0, 10)}... - ${h.balance} tokens`)
    })
  } else {
    console.log(`   (No holders with ALL 11 tokens from 10-20)`)
  }
...
// Run tests
testClickCreateSnapshots().catch(console.error)
