  ...
  try {
    ...
    const collections = await db.prepare(`
      SELECT
        ...
        COUNT(DISTINCT cs.address) as holderCount,
        COUNT(DISTINCT cs.token_id) as uniqueTokens,
        SUM(CAST(cs.balance AS BIGINT)) as totalSupply,
        MAX(e.block_timestamp) as lastActivityAt
      FROM contracts c
      LEFT JOIN current_state cs ON LOWER(c.address) = LOWER(cs.contract_address)
        AND CAST(cs.balance AS BIGINT) > 0
